...
def index():
    ...
    owned_stocks = db.execute("SELECT * FROM stocks WHERE user_id = ? AND quantity>0", user_id)
    total = user["cash"]
    cash = usd(user["cash"])
    ...
    return render_template("index.html", username = user["username"], cash = cash, total = usd(total), stocks= owned_stocks)
